...
public class TextStory : MonoBehaviour
{
    ...
    IEnumerator Texttext()
    {
        ...
        {
            Back.sprite = BackGround[23];
            character.sprite = Character[1];
            yield return StartCoroutine(NormalChat("사회자", "지금부터 ! 제 3회 플림의 예선을 시작합니다 !!!!"));
            Back.sprite = BackGround[0];
            yield return StartCoroutine(NormalChat("   ", "와아아아아아아 !!!!!!!!!!"));
            yield return StartCoroutine(NormalChat("다빈", "저번 합숙을 계기로 우리는 서로에게 한층 더 가까워지는데 성공했어. \n서로에 대한 신뢰가  쌓이니 연습을 할때도 더 자연스러운 느낌으로 흘러갈 수 있게되었지!"));
            yield return StartCoroutine(NormalChat("다빈", "우리를 바라보는 모두에게 이 말을 전하고 싶어 ! 오늘의 내가 내일의 나를 만든다 ! \n드디어 시작된 플림의 예선전 우리 록 크라운이 참가자로 시작해서 우승자인 \n우리를 만들 수 있을까 ? "));
            yield return StartCoroutine(NormalChat("다빈", "지금 부터는 플림에서의 이야기와 ...그 후의 이야기들이 담겨있어 그럼 다시 \n시작해보자 ! Let's Rock !"));
            Back.sprite = BackGround[23];
            yield return StartCoroutine(NormalChat("다빈", "우와아...예선전인데도 이렇게 엄청난 스테이지를 사용한다니..."));
            yield return StartCoroutine(NormalChat("하룬", "이 정도 크기면 관객이 몇명이나 들어올 수 있는거야??! 하나, 둘, 셋, 넷…"));
            yield return StartCoroutine(NormalChat("설하", "Amazing ! But...조금 떨리기 시작했어요."));
            yield return StartCoroutine(NormalChat("지윤", "떨리는게 당연해 그런데 잘 생각해봐 그 떨림이 벅차오르는 마음에서부터 \n나오는건지 아니면 두려움에서 나오는 건지 내가 보기에 설하의 마음은 이미 노래하고 있어"));
            yield return StartCoroutine(NormalChat("하룬", "응 ~! 얘들아 6개월 동안에 지옥 단련을 생각해봐 이정도 무대쯤은 충분히...\n아니 ? 오히려 너무 작은거 아니야 ? !"));
            yield return StartCoroutine(NormalChat("아라", "맞아 ! 나도 점점 불타오르기 시작했어 !"));
            yield return StartCoroutine(NormalChat("다빈", "히힛...모두들 반짝이고 있잖아 ?"));
            yield return StartCoroutine(NormalChat("지윤", "응? 다빈아 뭐라고?"));
            yield return StartCoroutine(NormalChat("다빈", "히힛 ~ 아니야 ! 먼저 간다 늦게 들어오면 참가 못하니까 빨리 와 ~!"));
            yield return StartCoroutine(NormalChat("아라", "다빈이 엄청 신났네"));
            yield return StartCoroutine(NormalChat("설하", "Yes 저 뒷모습을 보면 어디든 따라갈 수 있을 것 같아요 !"));
            yield return StartCoroutine(NormalChat("지윤", "좋아 그럼 우리도 따라갈까?"));
            yield return StartCoroutine(NormalChat("아라, 설하, 하룬, 지윤", "우리들의  리더를 !"));
            Back.sprite = BackGround[24];
            yield return StartCoroutine(NormalChat("참가자 1", "아아 ~ 라라라라라 ~"));
            yield return StartCoroutine(NormalChat("참가자 2", "( 띵 ~ 띠이잉 ~ )"));
            yield return StartCoroutine(NormalChat("다빈", "우와...이렇게나 많이 참가한거야?"));
            yield return StartCoroutine(NormalChat("지윤", "후...플림은 그런 대회니까 매번 새로운 시즌이 열릴때마다 참가자가 늘고 있지"));
            yield return StartCoroutine(NormalChat("설하", "이러고 있을 시간이 없어요 !  everyone 우리도 연습해요 !"));
            yield return StartCoroutine(NormalChat("하룬", "오 ! 적극적인데?"));
            yield return StartCoroutine(NormalChat("지윤", "좋아 무대 시작까지는 아직 시간이 좀 남았으니까"));
            yield return StartCoroutine(NormalChat("아라", "끄응..."));
            yield return StartCoroutine(NormalChat("지윤", "응? 아라야 왜그래?"));
            yield return StartCoroutine(NormalChat("아라", "별건 아니구...아까부터 뭔가를 까먹은 느낌이 든단 말이지..."));
            yield return StartCoroutine(NormalChat("설하", "What? 이제와서 돌아가긴 늦었다구요 !"));
            yield return StartCoroutine(NormalChat("아라", "아냐...두고 온게 아니야...같이 왔는데 잃어버린 거지이이이이...\n어어어어어????!!!!!!!!!!!! 다빈이! 다빈이가 없어요!!!"));
            yield return StartCoroutine(NormalChat("지윤", "뭐라고?!!"));
            yield return StartCoroutine(NormalChat("하룬", "큰일이네 이렇게 큰 건물에서 미아라도 되버리면..."));
            yield return StartCoroutine(NormalChat("설하", "No...다빈이라면 아까부터 저기서 모르는 사람이랑 놀고 있는 것 같은데..."));
            yield return StartCoroutine(NormalChat("다빈", "우와 ! 그럼 친구들끼리 제주도에서 부터 오신거에요?"));
            yield return StartCoroutine(NormalChat("참가자 3", "응 모두 음악을 좋아하거든 !  "));
            yield return StartCoroutine(NormalChat("다빈", "멋있어요 ! 응원할께요 !"));
            yield return StartCoroutine(NormalChat("참가자 3", "응 ! 너도 힘내 !"));
            yield return StartCoroutine(NormalChat("지윤", "...도대체 어떻게 되먹은 친화력인거야...?"));
            yield return StartCoroutine(NormalChat("아라", "하...하...예전부터 저런 성격이라"));
            yield return StartCoroutine(NormalChat("하룬", "뭐 어때 그럼 우리가 대신 자리 잡아두고 기다리자"));
            yield return StartCoroutine(NormalChat("설하", "Ok !"));
            yield return StartCoroutine(NormalChat("다빈", "좋아... 다들 빛나고 있어 모두...보석같아...!"));
            yield return StartCoroutine(NormalChat("다빈", "안녕하세요 ~"));
            yield return StartCoroutine(NormalChat("참가자 5", "어 그래 !"));
            yield return StartCoroutine(NormalChat("다빈", "안녕 안녕 ! "));
            yield return StartCoroutine(NormalChat("참가자 5", "안녕하세요 !"));
            yield return StartCoroutine(NormalChat("다빈", "즐겁다 ! 사람들은 만나면 만날수록 무대가 기대되.. 못참겠다아아 !! 어?"));
            yield return StartCoroutine(NormalChat("???", "..."));
            yield return StartCoroutine(NormalChat("다빈", "저 사람은 빛나고 있지...않아? 그치만...분명히 반짝거림이 보였는데..."));
            yield return StartCoroutine(NormalChat("다빈", "한번 다가가 봐야겠다. 저기 안녕하세요 !"));
            yield return StartCoroutine(NormalChat("???", ". . ."));
            yield return StartCoroutine(NormalChat("다빈", "( ...음...잘 안들렸나? 다시한번...! ) 안녕하세요 !"));
            yield return StartCoroutine(NormalChat("???", "( 다빈이를 살짝 째려본다 )"));
            yield return StartCoroutine(NormalChat("다빈", "( 으아아아 째려보잖아 무서워... ) 아..안녕하세요...다름이 아니라 저도 \n참가자라서 인사라도 해둘까 해서...하하..."));
            yield return StartCoroutine(NormalChat("???", "하아...인사해서 어쩔건데."));
            yield return StartCoroutine(NormalChat("다빈", "에?"));
            yield return StartCoroutine(NormalChat("???", "인사해서 어쩔꺼냐고."));
            yield return StartCoroutine(NormalChat("다빈", "그..그야...서로 잘 해보자는 의미로...그 그리고 친해지면 더 좋으니까..."));
            yield return StartCoroutine(NormalChat("???", "하 친해져? 야 정신차려 여긴 대회야 경쟁이라고 친해져서 좋을게 뭐가 있는데 \n아아 ~ 알겠다 뭐 서로 친해져서 누구 한명이 떨어지게 되면 그 때 질질 짜려고 ? "));
            yield return StartCoroutine(NormalChat("다빈", "사과해..."));
            yield return StartCoroutine(NormalChat("???", "뭐라고 ? 말을 크게해야 들릴거 아니야 그렇게 작게 말하면 누ㄱ.."));
            yield return StartCoroutine(NormalChat("다빈", "사과하라고 !!!"));
            yield return StartCoroutine(NormalChat("???", "하아…"));
            yield return StartCoroutine(NormalChat("다빈", "우리의 꿈을 마음대로 짓밟지 마 ! 우리들의 노력은...우리들의 다짐은..."));
            yield return StartCoroutine(NormalChat("???", "수아야 무슨일 있어?"));
            yield return StartCoroutine(NormalChat("수아", "아...지우야 왔구나...아무일도 아니야 가자 갑자기 뭐라도 마시고 싶네"));
            yield return StartCoroutine(NormalChat("지우", "...뒤에 계신분이랑 이야기 하고 있었던 거 아니야?"));
            yield return StartCoroutine(NormalChat("수아", "아...아니야 그만 가자"));
            yield return StartCoroutine(NormalChat("지우", "으응.."));
            yield return StartCoroutine(NormalChat("   ", "( 수아가 나가고 지우는 다빈이에게 인사를하고 나간다. )"));
            yield return StartCoroutine(NormalChat("다빈", "정말...뭐야 저 사람들…"));
            yield return StartCoroutine(NormalChat("지윤", "다빈아 ! 무슨일이야 "));
            yield return StartCoroutine(NormalChat("하룬", "큰소리가 나길래 와봤어"));
            yield return StartCoroutine(NormalChat("다빈", "으응 아무일도 아니야 단지...우리가 꼭 이겨야하는 이유가 하나 생겼을 뿐이지...!"));
            yield return StartCoroutine(NormalChat("지윤", "응?"));
            yield return StartCoroutine(NormalChat("다빈", "빨리 가자 이제 정말 연습해야될 시간이야 !"));
            yield return StartCoroutine(NormalChat("하룬", "아 ! 벌써 시간이 가자 지윤아 !"));
            yield return StartCoroutine(NormalChat("지윤", "그...그래"));
            Back.sprite = BackGround[23];
            yield return StartCoroutine(NormalChat("사회자", "이번 무대도 정말 엄청난 퀄리티를 뽐내는 그런 무대였습니다 ! \n이번 무대가 마음에 드셨다면 투표하기를 눌러주세요! \n자 그럼 다음 참가자입니다!"));
            yield return StartCoroutine(NormalChat("설하", "one...two...이제 곧 저희 순서네요...!"));
            yield return StartCoroutine(NormalChat("아라", "응 ! 두근 거리기 시작했어...!"));
            yield return StartCoroutine(NormalChat("사회자", "오호? 이번 무대에 오르는 참가자는 플림에 나오기 위해 새로 그룹을 만들었다고 \n하는데요? 어떤 퍼포먼스를 보여줄지 기대가 되는군요 자 소개합니다!"));
            yield return StartCoroutine(NormalChat("사회자", "\" 더스틴 \" !"));
            yield return StartCoroutine(NormalChat("다빈", "...?! 그 사람이다 !"));
            yield return StartCoroutine(NormalChat("지윤", "아는 분들이야?"));
            yield return StartCoroutine(NormalChat("다빈", "응...대기실에서 잠깐 마주쳤어"));
            yield return StartCoroutine(NormalChat("하룬", "아 ! 그러면 저 사람이 다빈이랑 부딫혔던 사람 이라는거지?"));
            yield return StartCoroutine(NormalChat("다빈", "응..."));
            yield return StartCoroutine(NormalChat("설하", "으느느느느느늣 Crazy ! 감히 다빈이를 건드려? 실력한번 보자구요 !"));
            Back.sprite = BackGround[10];
            yield return StartCoroutine(NormalChat("수아", "가자 지우야"));
            yield return StartCoroutine(NormalChat("지우", "응"));
            Back.sprite = BackGround[23];
            yield return StartCoroutine(NormalChat("(함성소리)", "와!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"));
            yield return StartCoroutine(NormalChat("사회자", "…"));
            yield return StartCoroutine(NormalChat("관객들", "…"));
            yield return StartCoroutine(NormalChat("관객들", "와아아아아아아아아 !!!!!!!!!!!"));
            yield return StartCoroutine(NormalChat("사회자", "와아아아아아아아아 !!!!!!!!!!!"));
            yield return StartCoroutine(NormalChat("사회자", "이럴 수가 정말 새로 생긴 팀인가 싶을정도로 엄청난 퍼포먼스를 보여준 \n더스틴 이번 무대를 통해 엄청난 존재감을 들어냈습니다!!!"));
            yield return StartCoroutine(NormalChat("지우", "다들 좋아하네..."));
            yield return StartCoroutine(NormalChat("수아", "이런 함성 들을필요 없어. 어차피 들꽃이 진 자리는 아무도 기억 못하는걸 가자."));
            Back.sprite = BackGround[10];
            yield return StartCoroutine(NormalChat("다빈", "대..단해..."));
            yield return StartCoroutine(NormalChat("설하", "holy ! moly ! What is this !"));
            yield return StartCoroutine(NormalChat("다빈", "빛나...빛나고 있지만...어두워..."));
            yield return StartCoroutine(NormalChat("지윤", "..."));
            yield return StartCoroutine(NormalChat("하룬", "뭐 ! 일단은 눈앞에 있는 문제부터 해결하자고 !"));
            yield return StartCoroutine(NormalChat("아라", "응 ! 기다려왔잖아 이 무대를 !"));
            yield return StartCoroutine(NormalChat("다빈", "좋았어 ! 가자 !"));
            Back.sprite = BackGround[0];
            yield return StartCoroutine(NormalChat("   ", "♪~ ♬~~ ♩ ♪ ♪~ ♬~"));
            Back.sprite = BackGround[23];
            yield return StartCoroutine(NormalChat("관객들", "와아아아아아아아아 !!!!!!!!!!!"));
            yield return StartCoroutine(NormalChat("사회자", "와아아아아아아아아 !!!!!!!!!!! 이야 ! 더스틴에 이어 또 등장한 신생 그룹이\n 관객분들의 마음을 또 사로잡았다!!! 록 크라운! 더스틴과 마찬가지로 \n엄청난 무대를 선보입니다! "));
            yield return StartCoroutine(NormalChat("다빈", "후우...힘들었다."));
            yield return StartCoroutine(NormalChat("설하", "Yes 힘이 쭈욱 빠지는 느낌이네요."));
            yield return StartCoroutine(NormalChat("아라", "그러게..."));
            yield return StartCoroutine(NormalChat("하룬", "그래도 재밌지 않았어?"));
            yield return StartCoroutine(NormalChat("다빈", "ㅎㅎ 맞아 ! 역시 록 아이돌은 최고야 !"));
            yield return StartCoroutine(NormalChat("지윤", "응 ! 이제 남은건 믿고 기다리는 것 뿐이야. 모두 수고했어"));
            yield return StartCoroutine(NormalChat("다빈", "응응 !"));
            Back.sprite = BackGround[13];
            yield return StartCoroutine(NormalChat("   ", "따르릉 따르릉 따르릉"));
            yield return StartCoroutine(NormalChat("지윤", "흠냐...흐응~ 누구야앙...여보세요..."));
            yield return StartCoroutine(NormalChat("다빈", "지윤선배 !!!!"));
            yield return StartCoroutine(NormalChat("지윤", "으엑 깜짝아 ! 무슨일이야 다빈아..."));
            yield return StartCoroutine(NormalChat("다빈", "지금이 자고 있을때야?!! 빨리 인터넷 한번 봐바 ! 가글이든 나이뻐든 \n아무대나 들어가봐 !!"));
            yield return StartCoroutine(NormalChat("지윤", "무슨소리지..."));
            yield return StartCoroutine(NormalChat("   ", "( 지윤이가 핸드폰을 만지작 거린다. )"));
            yield return StartCoroutine(NormalChat("지윤", "..에? 에에??! 에에에엥에에에에에?!!!!!!!!!"));
            yield return StartCoroutine(NormalChat("다빈", "선배 우리가 해냈어 ! 지금 어딜가도 록크라운의 이야기 뿐이야 ! \n추가로 더스틴도 있지만..."));
            yield return StartCoroutine(NormalChat("지윤", "이게 정말이야...? 나 아직 꿈에서 깨지 못한건가?"));
            yield return StartCoroutine(NormalChat("다빈", "지윤선배 !! "));
            yield return StartCoroutine(NormalChat("지윤", "다빈아 !!"));
            Back.sprite = BackGround[0];
            yield return StartCoroutine(NormalChat("다빈", "플림의 예선 이후로 우리 록크라운은 더스틴과 함께 우승 후보라 불리게 되며 \n엄청난 주목을 받게되었어."));
            yield return StartCoroutine(NormalChat("다빈", "세계적인 대회인 만큼 어딜가나 우리 얘기 뿐이었고 그 기세를 그대로 이어가 \n본선에도 진출할 수 있게되었지."));
            yield return StartCoroutine(NormalChat("다빈", "본선에서도 우리의 기세는 꺾일줄을 몰랐어 꽤나 인지도가 있는 그룹을 \n상대로 승리를 하기도 하고 아예 팬클럽까지 있는 그룹을 상대로도 이겨봤어."));
            yield return StartCoroutine(NormalChat("다빈", "그렇게 승승장구하며 올라온 우리는 이제 꿈에 그리던 결승 무대만을 앞두고 있어 !"));
            yield return StartCoroutine(NormalChat("다빈", "상대는 역시 더스틴 지금부터가 본 게임이야...!"));
            Back.sprite = BackGround[26];
            yield return StartCoroutine(NormalChat("   ", "짝!"));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "뭐하는거야 ! 내가 니들한테 얼마나 기대하고 있는지 모르는거냐?"));
            Back.sprite = BackGround[25];
            yield return StartCoroutine(NormalChat("수아", "...죄송합니다"));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "최고의 그룹이라고 불리던 녀석들이잖아? 저런 햇병아리 같은 애들이랑 \n동급이라고 불리는게 쪽팔리지도 않냐 ? "));
            yield return StartCoroutine(NormalChat("수아", "죄송합니다.."));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "이거 이거 ~ 요즘 좀 느슨해 진 것 같은데 다시 압박좀 넣어줄까?"));
            yield return StartCoroutine(NormalChat("수아", "..."));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "이것봐라? 대답안해?"));
            yield return StartCoroutine(NormalChat("수아", "...하"));
            yield return StartCoroutine(NormalChat("지우", "수...수아야"));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "어쭈? 한숨을 쉬어 ? 뭐 어쩌게 이제 못해먹겠습니다 ~ 하면서 나가버리게? \n뭐 할 수 있으면 해보든가 나는 너희들 버리고 그 거지같은 동네 \n밀어버리는게 더 이득이라고"));
            yield return StartCoroutine(NormalChat("지우", "죄송해요...수아가 최근에 많이 움직이다 보니까 힘들어서 그런 것 같아요.\n 제가 대신 사과드릴께요..."));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "참 아무튼 잘 생각하고 행동하라 이거야 가봐 ~"));
            yield return StartCoroutine(NormalChat("지우", "네...감사합니다...가자 수아야..."));
            yield return StartCoroutine(NormalChat("A.엔터테이먼트 사장", "아 참 그리고 플림에서 확실하게 우승하지 못하면 \n니들이랑 한 약속도 끝이고 그럼 그 다음은 알고 있지 ? "));
            yield return StartCoroutine(NormalChat("지우", "...네 사장님 가자..."));
            Back.sprite = BackGround[0];
            yield return StartCoroutine(NormalChat("수아", "지우야…"));
            yield return StartCoroutine(NormalChat("지우", "응"));
            yield return StartCoroutine(NormalChat("수아", "우리 플림에서 우승하기만 하면 다시 돌아갈 수 있는거겠지..."));
            yield return StartCoroutine(NormalChat("지우", "그럼 당연하지"));
            yield return StartCoroutine(NormalChat("수아", "응 꼭 우승하자 우승해서...다시 우리들로 돌아가자"));
            yield return StartCoroutine(NormalChat("지우", "응 꼭 그러자."));
        }
